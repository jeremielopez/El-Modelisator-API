...
/**
 * Basic REST service used to manage an entity with the database.
 *
...
export default class RestService<T extends RestEntity> {
  ...
  constructor(protected entityClass: new () => T,
    ...
    this.repository = this.entityManager.getMongoRepository(this.entityClass);
  }
   ...
   * @returns {Promise<RestEntity>}
   */
  get(id: ObjectID, embeds: string[] = [], joins: Join[] = []): Promise<T> {
   ...
   * @returns {Promise<RestEntity>}
   */
  update(id: ObjectID, data: any, embeds: string[] = [], joins: Join[] = []): Promise<T> {
